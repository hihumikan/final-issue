...
int main(void){
	...
	for( i = 0; i < 3; i++ )
        ...
        //ここからジャンケン
        playerJK = 0;//0を代入しておく
        while(playerJK != 1){//無限ループ
        printf("--ジャンケンをします、出したい手の数字を教えてください--\n");
        printf("1.グー 2.チョキ 3.パー 4.反則技\n");
        printf("出したい手:");
        scanf("%d",&playerJK);//ジャンケンを選ばせる。
        if(playerJK > 0 && playerJK < 4){//4以下ならばループ脱出
            break;  
        }

        if(playerJK == 4){//反則技を使った場合、線形代数をやらせて、答えがあってるまでloop
        ...
        if(playerJK == compJK){//あいこならばゲーム終了
            printf("女子児童:あいこだったね。。。つまんなかったしこのゲーム終了するね\n");
            return 0;
        } else if(playerJK == 1 && compJK == 2){//プレイヤーが勝つ場合ゲーム終了
            printf("女子児童:負けたーーー！！！つまんない！！！ゲーム終了させてやる！!!\n");
            return 0;
        } else if(playerJK == 1 && compJK == 3){//プレイヤーが負ける場合
            printf("女子児童:わーい勝った勝ったー！\n");
        } else if(playerJK == 2 && compJK == 1){//プレイヤーが負ける場合
            printf("女子児童:わーい勝った勝ったー！\n");
        } else if(playerJK == 2 && compJK == 3){//プレイヤーが勝つ場合ゲーム終了
            printf("女子児童:負けたーーー！！！つまんない！！！ゲーム終了させてやる！!!\n");
            return 0;
        } else if(playerJK == 3 && compJK == 1){//プレイヤーが勝つ場合ゲーム終了
            printf("女子児童:負けたーーー！！！つまんない！！！ゲーム終了させてやる！!!\n");
            return 0;
        } else if(playerJK == 3 && compJK == 2){//プレイヤーが負ける場合
            printf("女子児童:わーい勝った勝ったー！\n");
        }
    ...
    while(1){//無限ループ
        ...
        for(i=0;i<x;i++){
            for(j=0;j<y;j++){
                    ...
                    if(mae[i][j]==1){//1ならば*を表示させるとする
                        printf("*");
                    } else if(mae[i][j]==0){//0ならば空白表示
                        printf(" ");
                    }
                    ...
                    if(anpi < 2){//周囲に1が1個,0個ならば
                        usiro[i][j]=0;//死ぬ
                    }else if(anpi == 2){//ちょうど２個ならば
                        usiro[i][j]=mae[i][j];//維持
                    }else if(anpi == 3){//ちょうど3個ならば
                        usiro[i][j]=1;//新たな命
                    }else{//それ以外ならば
                        usiro[i][j]=0;//死ぬ
                }
